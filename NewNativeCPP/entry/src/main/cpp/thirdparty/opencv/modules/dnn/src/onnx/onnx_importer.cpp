...
static DictValue parse(const ::google::cv_protobuf::RepeatedField< ::google::cv_protobuf::int64>& src) {
    std::vector<int32_t> dst(src.size());
    convertInt64ToInt32(src, dst, src.size());
    return DictValue::arrayInt(&dst[0], src.size());
}

static DictValue parseStr(const ::google::cv_protobuf::RepeatedPtrField< ::std::string>& src) {
...
LayerParams ONNXImporter::getLayerParams(const opencv_onnx::NodeProto& node_proto)
{
    ...
    for(int i = 0; i < node_proto.attribute_size(); i++)
    {
        ...
        try
        {
            ...
            {
                ::google::cv_protobuf::int64 src = attribute_proto.i();
