...
using ::google::cv_protobuf::RepeatedField;
using ::google::cv_protobuf::RepeatedPtrField;
...
void printLayerAttr(const tensorflow::NodeDef &layer)
{
    ...
    std::cout << std::endl;
    google::cv_protobuf::Map<std::string, tensorflow::AttrValue> attr
            = layer.attr();
    for (google::cv_protobuf::Map<std::string, tensorflow::AttrValue>::const_iterator ai = attr.begin();
         ai != attr.end(); ++ai)
    {
        std::cout << ai->first << ":";
...
{
    google::cv_protobuf::Map<std::string, tensorflow::AttrValue> attr = layer.attr();
    ...
    if (type == "FusedResizeAndPadConv2D")
    {
        ...
        if (hasLayerAttr(layer, "resize_align_corners"))
        {
            ...
            layer.mutable_attr()->insert(
                    ::google::cv_protobuf::MapPair<std::string, tensorflow::AttrValue>("align_corners",
...
void writeTextGraph(const String& _model, const String& output)
{
    ...
    std::string content;
    google::cv_protobuf::TextFormat::PrintToString(net, &content);
